...
module niosv_pd_altera_merlin_master_translator_193_lgcew2q #(
   parameter
      // widths
      AV_ADDRESS_W                = 32,
      AV_DATA_W                   = 32,
      ...
      if (SYNC_RESET == 0) begin : async_rst5
         always @(posedge clk or posedge reset) begin
            if (reset) begin
               ...
               if (
                  (av_waitrequest_r===1) && // When waitrequest is asserted //avoid printing error message when signal is X or Z
                  (
                  ...
                  ((av_write_r===1) | (av_read_r===1)) &&         // Check only when : previously initiated a write/read
                  (!USE_CHIPSELECT | (av_chipselect_r===1 )) //                   and chipselect was asserted (or unused)
               ) begin
         ...
         always @(posedge clk) begin
            if (internal_sclr) begin
               ...
               if (
                 ( av_waitrequest_r===1) && // When waitrequest is asserted
                  (
                     (av_write                  != av_write_r) ||   // Checks that : Input controls/data does not change
                     (av_read                   != av_read_r)  ||
                     (av_lock                   != av_lock_r)  ||
